...
import { getDetallesSolicitudes, getSolicitudDetalleById, createSolicitud, updateSolicitud } from "../../../services/solicituddetalles";
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (id) {
      const solicitud = await getSolicitudDetalleById(parseInt(id));
      if (!solicitud) {
    ...
    const solicitudes = await getDetallesSolicitudes();
