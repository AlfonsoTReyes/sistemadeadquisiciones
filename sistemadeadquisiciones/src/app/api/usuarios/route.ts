...
export async function POST(req: NextRequest) {
  try {
    const { nombre, email, password, rol, nomina, descriptor, emailUsuario, secretaria} = await req.json();
    // Convertir el descriptor facial a JSON si es necesario (asegurando que es un Float32Array)
    const descriptorJSON = JSON.stringify(Array.from(new Float32Array(descriptor)));
    const nuevoUsuario = await createUsuario(nombre, email, password, rol, nomina, descriptorJSON, emailUsuario, secretaria);
...
export async function PUT(req: NextRequest) {
  ...
  try {
    const { id_usuario, nombre, email, rol, nomina, password, emailUsuario, secretaria} = await req.json();
    if (!id_usuario) {
      return NextResponse.json({ message: 'ID no proporcionado' }, { status: 400 });
    }
    ...
    const usuarioActualizado = await updateUsuario(id_usuario, { nombre, email, rol, nomina, emailUsuario, secretaria}); 
    if (!usuarioActualizado) {
      return NextResponse.json({ message: 'Usuario no encontrado' }, { status: 404 });
    }
