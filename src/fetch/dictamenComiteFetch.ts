// src/fetch/dictamenComiteFetch.ts (o donde prefieras)

const DICTAMENES_API_URL = '/api/dictamenes';
...
/**
 * Obtiene la lista de dictámenes formateada para usar en un <select>.
 * @returns Promise<SelectOption[]>
 */
export const fetchDictamenesForSelect = async (): Promise<SelectOption[]> => {
    const logPrefix = "FETCH fetchDictamenesForSelect:";
    const apiUrl = `${DICTAMENES_API_URL}?forSelect=true`;
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(data?.message || `Error ${response.status}: No se pudo obtener la lista de dictámenes.`);
        }
