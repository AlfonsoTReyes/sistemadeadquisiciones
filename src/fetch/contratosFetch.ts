...
import { ContratoCreateData, ContratoUpdateData, ContratoEnLista, ContratoDetallado } from '@/types/contrato'; // Ajusta la ruta
 ...
 */
export const fetchProveedorByUserId = async (idUsuarioProveedor: number): Promise<ProveedorDetallado | null> => {
    ...
    try {
        ...
        if (response.status === 404) {
            console.warn(`${logPrefix} Proveedor no encontrado (404) para este usuario.`);
        ...
        if (!response.ok) {
            ...
            throw new Error(data?.message || `Error ${response.status}: No se pudo obtener el perfil del proveedor.`);
        }
        if (!data || typeof data !== 'object' || data === null) {
            console.error(`${logPrefix} Error GET ${apiUrl}: Respuesta inválida del servidor. Response:`, data);
            throw new Error("Respuesta inesperada del servidor al obtener perfil del proveedor.");
        }
 ...
 */
export const fetchProveedoresForSelect = async (): Promise<{ id: number; label: string }[]> => {
    ...
    try {
        ...
        if (!response.ok) {
            console.error(`${logPrefix} Error GET ${apiUrl}: Status ${response.status}. Response:`, data);
            throw new Error(data?.message || `Error ${response.status}: No se pudo obtener la lista de proveedores.`);
        }
 ...
 */
export const fetchContracts = async (idProveedor?: number): Promise<ContratoEnLista[]> => {
    ...
    if (typeof idProveedor === 'number' && !isNaN(idProveedor)) {
        ...
        console.log(`${logPrefix} Calling GET ${apiUrl} (filtrando para proveedor ${idProveedor})`);
    } else {
        console.log(`${logPrefix} Calling GET ${apiUrl} (obteniendo todos - ¡sin filtro de proveedor!)`);
    }
    ...
    try {
        ...
        if (!response.ok) {
            console.error(`${logPrefix} Error GET ${apiUrl}: Status ${response.status}. Response:`, data);
            throw new Error(data?.message || `Error ${response.status}: No se pudo obtener la lista de contratos.`);
        }
        if (!Array.isArray(data)) {
             console.error(`${logPrefix} Error GET ${apiUrl}: Respuesta no es un array. Response:`, data);
             throw new Error("Respuesta inesperada del servidor al obtener contratos.");
        }
 ...
 */
export const fetchContractDetails = async (idContrato: number): Promise<ContratoDetallado> => {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            if (response.status === 404) {
                 throw new Error(data?.message || `Contrato con ID ${idContrato} no encontrado.`);
            }
            throw new Error(data?.message || `Error ${response.status}: No se pudieron obtener los detalles del contrato.`);
        }
         if (!data || typeof data !== 'object' || data === null) {
             console.error(`${logPrefix} Error GET ${apiUrl}: Respuesta inválida del servidor. Response:`, data);
             throw new Error("Respuesta inesperada del servidor al obtener detalles del contrato.");
         }
 ...
 */
export const createContractRequest = async (contratoData: ContratoCreateData): Promise<{ id_contrato: number }> => {
    ...
    if (!contratoData || typeof contratoData.id_proveedor !== 'number' || !contratoData.objeto_contrato || !contratoData.monto_total) {
    ...
    try {
        ...
        if (!response.ok) {
            console.error(`${logPrefix} Error POST ${apiUrl}: Status ${response.status}. Response:`, data);
            // Manejar errores específicos de la API (ej: 400 Bad Request, 409 Conflict)
            throw new Error(data?.message || `Error ${response.status}: No se pudo crear el contrato.`);
        }
        if (!data || typeof data.id_contrato !== 'number') {
            console.error(`${logPrefix} Error POST ${apiUrl}: Respuesta inválida tras creación exitosa. Response:`, data);
 ...
 */
export const updateContractRequest = async (idContrato: number, contratoData: ContratoUpdateData): Promise<ContratoDetallado> => {
    ...
    if (contratoData.monto_total !== undefined && isNaN(parseFloat(contratoData.monto_total))) {
    ...
    try {
        ...
        if (!response.ok) {
            console.error(`${logPrefix} Error PUT ${apiUrl}: Status ${response.status}. Response:`, data);
             if (response.status === 404) {
                 throw new Error(data?.message || `Contrato con ID ${idContrato} no encontrado para actualizar.`);
            }
            throw new Error(data?.message || `Error ${response.status}: No se pudo actualizar el contrato.`);
        }
         if (!data || typeof data !== 'object' || data === null) {
             console.error(`${logPrefix} Error PUT ${apiUrl}: Respuesta inválida del servidor tras actualización. Response:`, data);
             throw new Error("Respuesta inesperada del servidor al actualizar el contrato.");
         }
