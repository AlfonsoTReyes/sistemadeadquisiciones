...
export default function NotificationManager({ userId, idRol }: NotificationManagerProps) {
    ...
    return (
        <div className="relative">
            ...
            {mostrarDropdown && (
                <div
                    className="fixed right-4 top-16 w-80 bg-white text-gray-800 rounded-lg shadow-lg border border-gray-200 z-50 overflow-hidden"
                            ...
                            {notificaciones.length === 0 ? (
                                ...
                                notificaciones.map((n) => (
                                    <li
                                        ...
                                        className={`px-4 py-3 hover:bg-gray-100 transition duration-150 ease-in-out ${n.fue_leida ? 'text-gray-500' : 'font-medium bg-blue-50'}`}
                                    >
                                        ...
                                        {n.fecha_creacion && (
                                            <p className="text-xs text-gray-400 mt-1">
                                                {new Date(n.fecha_creacion).toLocaleString('es-MX', { dateStyle: 'short', timeStyle: 'short' })}
                                            </p>
                                        )}
                                    </li>
                                ))
