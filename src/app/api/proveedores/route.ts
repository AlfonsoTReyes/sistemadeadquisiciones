...
import {
    ...
    buscarProveedoresPorTermino
} from '../../../services/proveedoresservice'; // Ajusta la ruta
import { Proveedor } from '@/types/proveedor';
// --- GET ---
export async function GET(req: NextRequest) {
    const logPrefix = "ROUTE GET /api/proveedores:";
    try {
        ...
        const searchTerm = searchParams.get('search'); // <-- LEER NUEVO PARÁMETRO DE BÚSQUEDA
        ...
        console.error(`${logPrefix} Generic Error:`, error);
        const status = error.message?.includes("no encontrado") ? 404 : 500;
        return NextResponse.json({ message: error.message || 'Error general al obtener proveedor(es)', error: error.toString() }, { status });
    }
}
// --- PUT ---
