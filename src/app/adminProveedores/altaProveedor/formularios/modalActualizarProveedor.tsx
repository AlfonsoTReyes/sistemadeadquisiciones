// src/app/adminProveedores/altaProveedor/formularios/modalActualizarProveedor.tsx
'use client';
...
// RepresentanteEditable interface removed as it was equivalent to RepresentanteLegalOutput
...
interface ModalUpdatePayload {
    ...
    rfc: string;
    giro_comercial?: string | null;
    actividadSat: string; // Asegúrate que el backend espera camelCase si así lo envías
    correo: string;
    calle: string;
    numero: string;
    colonia: string;
    ...
    proveedorEventos?: boolean;
    // Campos Físicos
    nombre?: string; // Corresponde a form.nombre que viene de nombre_fisica
    apellido_p?: string; // Corresponde a form.apellido_p que viene de apellido_p_fisica
    apellido_m?: string | null; // Corresponde a form.apellido_m que viene de apellido_m_fisica
    curp?: string;
    // Campos Morales
    razon_social?: string;
...
const ModalActualizarProveedor: React.FC<ModalProps> = ({
    ...
    // --- Estados (MOVIDOS AL NIVEL SUPERIOR) ---
    const [form, setForm] = useState<ProveedorFormData>({
        // Inicializar con valores por defecto para evitar errores si proveedorData es null inicialmente
        rfc: '', nombre: '', apellido_p: '', apellido_m: '', curp: '', razon_social: '',
        giro_comercial: '', actividadSat: '', correo: '', calle: '', numero: '',
        colonia: '', codigo_postal: '', municipio: '', estado: '', telefono_uno: '',
        telefono_dos: '', pagina_web: '', camara_comercial: '',
        numero_registro_camara: '', numero_registro_imss: '', proveedorEventos: false,
    });
    const [representantes, setRepresentantes] = useState<RepresentanteLegalOutput[]>([]); // Usar RepresentanteLegalOutput
    ...
    useEffect(() => {
        if (proveedorData && proveedorData.id_proveedor) {
            ...
            setFormError(null);
            setRepError(null);
        } else if (isOpen) { // Solo advertir si el modal está abierto y no hay datos
            console.warn("Modal Admin useEffect: proveedorData inválido o falta id_proveedor, pero el modal está abierto.");
        }
    }, [proveedorData, isOpen]); // isOpen añadido como dependencia por si el modal se abre sin datos válidos

    // --- Early returns DESPUÉS de los hooks ---
    if (!isOpen) return null;
    if (!proveedorData) {
        // Esto no debería suceder si isOpen es true y el padre maneja bien los datos,
        // pero es una salvaguarda.
        return (
            <div className="fixed inset-0 bg-gray-800 bg-opacity-80 flex justify-center items-center z-50 p-4">
                <div className="bg-white rounded-lg p-6">Error: No hay datos del proveedor para mostrar.</div>
            </div>
        );
    }
    ...
    const handleAddRepresentante = () => {
        ...
        const nuevoRepresentante: RepresentanteLegalOutput = { // Usar RepresentanteLegalOutput
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        } else if (tipoProveedorOriginal === 'desconocido') {
            setFormError('No se puede actualizar un proveedor de tipo "desconocido". Por favor, contacte a soporte.'); return;
        } else {
        ...
        const payload: ModalUpdatePayload = { // CORREGIDO: any -> ModalUpdatePayload
            ...
            tipoProveedor: tipoProveedorOriginal as 'moral' | 'fisica', // Cast si estamos seguros que no es 'desconocido' aquí
            rfc: form.rfc,
        ...
        <div className="fixed inset-0 bg-gray-800 bg-opacity-80 flex justify-center items-center z-50 p-4">
            <div className="bg-white rounded-lg shadow-2xl p-6 md:p-8 w-full max-w-4xl max-h-[95vh] overflow-y-auto">
                <div className="flex justify-between items-center mb-6 border-b border-gray-200 pb-4">
                    ...
                    <button
                        onClick={onClose}
                        disabled={isLoading} // Usar prop isLoading
                ...
                <form onSubmit={handleSubmit} noValidate>
                    ...
                    {tipoProveedorOriginal === 'fisica' && (
                        <fieldset className={fieldsetStyle}>
                            <legend className={legendStyle}>Datos Persona Física</legend>
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                <div>
                                    <label htmlFor="modal_nombre" className={labelStyle}>Nombre(s) {requiredMark}</label>
                                    <input type="text" id="modal_nombre" name="nombre" value={form.nombre || ''} onChange={handleChange} required className={inputStyle} disabled={isLoading} />
                                </div>
                                <div>
                                    <label htmlFor="modal_apellido_p" className={labelStyle}>Apellido Paterno {requiredMark}</label>
                                    <input type="text" id="modal_apellido_p" name="apellido_p" value={form.apellido_p || ''} onChange={handleChange} required className={inputStyle} disabled={isLoading} />
                                </div>
                                <div>
                                    <label htmlFor="modal_apellido_m" className={labelStyle}>Apellido Materno</label>
                                    <input type="text" id="modal_apellido_m" name="apellido_m" value={form.apellido_m || ''} onChange={handleChange} className={inputStyle} disabled={isLoading} />
                                </div>
                            </div>
                            <div className="mt-4">
                                <label htmlFor="modal_curp" className={labelStyle}>CURP {requiredMark}</label>
                                <input type="text" id="modal_curp" name="curp" value={form.curp || ''} onChange={handleChange} required maxLength={18} minLength={18} className={inputStyle} disabled={isLoading} />
                            </div>
                        </fieldset>
                    )}
                            ...
                            <div className="mt-4 space-y-4">
                                ...
                                {representantes.length > 0 && (
                                    <div className="space-y-2 border-t pt-3 mt-3">
                                        {representantes.map((rep) => ( // index removido
                                            <div key={rep.id_morales} className="flex justify-between items-center bg-gray-100 px-3 py-2 rounded text-sm">
                                                ...
                                                <button type="button" onClick={() => handleRemoveRepresentante(rep.id_morales)} className="px-2 py-1 bg-red-500 text-white text-xs rounded hover:bg-red-600 disabled:opacity-50" title="Eliminar" disabled={isLoading}>Eliminar</button>
                                            </div>
                                        ))}
                                    </div>
                                )}
                                ...
                                <div className="border border-dashed border-indigo-300 p-4 rounded-md bg-indigo-50 mt-4">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                                        <div>
                                            ...
                                            <input type="text" id="nuevoRepNombre" name="nuevoRepNombre" value={nuevoRepNombre || ''} onChange={handleRepChange} className={`${inputStyle} text-sm`} disabled={isLoading} />
                                        </div>
                                        <div>
                                            <label htmlFor="nuevoRepApellidoP" className={`${labelStyle} text-xs`}>Apellido Paterno {requiredMark}</label>
                                            <input type="text" id="nuevoRepApellidoP" name="nuevoRepApellidoP" value={nuevoRepApellidoP || ''} onChange={handleRepChange} className={`${inputStyle} text-sm`} disabled={isLoading} />
                                        </div>
                                        <div>
                                            <label htmlFor="nuevoRepApellidoM" className={`${labelStyle} text-xs`}>Apellido Materno</label>
                                            <input type="text" id="nuevoRepApellidoM" name="nuevoRepApellidoM" value={nuevoRepApellidoM || ''} onChange={handleRepChange} className={`${inputStyle} text-sm`} disabled={isLoading} />
                                        </div>
                                    </div>
                                    <button type="button" onClick={handleAddRepresentante} className="mt-3 px-3 py-1 bg-indigo-500 text-white text-xs rounded hover:bg-indigo-600 disabled:opacity-50" disabled={!nuevoRepNombre || !nuevoRepApellidoP || isLoading}>+ Añadir</button>
                                </div>
                            </div>
                        </fieldset>
                    )}
                    <fieldset className={fieldsetStyle}>
                        <legend className={legendStyle}>Datos Generales y Fiscales</legend>
                        ...
                        <div className="mb-4">
                            <label htmlFor="modal_actividadSat" className={labelStyle}>
                                Actividad Económica (SAT) {requiredMark}
                            </label>
                            <input
                                type="text"
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                            ...
                            <div>
                                <label htmlFor="modal_numero_registro_camara" className={labelStyle}>No. Registro Cámara</label>
                            ...
                            <div>
                                <label htmlFor="modal_numero_registro_imss" className={labelStyle}>No. Registro IMSS</label>
                    ...
                    <fieldset className={fieldsetStyle}>
                        <legend className={legendStyle}>Dirección</legend>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    ...
                    <fieldset className={fieldsetStyle}>
                        <legend className={legendStyle}>Contacto</legend>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            <div>
                        ...
                        <div className="mb-1">
                            ...
                            <input type="url" id="modal_pagina_web" name="pagina_web" value={form.pagina_web || ''} onChange={handleChange} placeholder="https://ejemplo.com" className={inputStyle} disabled={isLoading} />
                        </div>
                    </fieldset>
                    ...
                    <div className="mt-8 pt-5 border-t border-gray-200 flex justify-end space-x-4">
                        ...
                        <button
                            type="submit"
                            disabled={isLoading || !!formError || !!repError}
                            ...
                            ${isLoading || formError || repError ? 'bg-indigo-400 cursor-not-allowed' : 'bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500'} 
                            disabled:opacity-60`}
                        >
                            {isLoading ? (
                                <>
