...
const REQUIRED_DOCS_FISICA: RequiredDoc[] = [
  ...
  { key: 'ID_PERSONAL', label: 'Identificación Oficial Vigente' },
...
interface GestionDocumentosProveedorProps {
  idProveedor: number;
  tipoProveedor: 'moral' | 'fisica' | string;
  onClose: () => void;
}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-3">
          ...
          <div>
            ...
            <input
              ...
              required={tipoDocumentoOtro.trim() !== ""}
          ...
          <div className="overflow-x-auto border rounded shadow-sm">
            <table className="min-w-full bg-white">
              <thead className="bg-gray-100">
                <tr>
                  <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nombre Original</th>
                  <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Tipo Documento</th>
                  <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Subido</th>
                ...
                {documentos.map(doc => (
                  <tr key={doc.id_documento_proveedor} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2 whitespace-nowrap text-sm">
                      <button
                        onClick={async () => {
                          if (window.confirm(`¿Seguro que deseas eliminar "${doc.nombre_original}"? Esta acción no se puede deshacer.`)) {
                            try {
                              setErrorGlobal(null); setSuccessMessage(null);
                              await deleteDocumentoProveedor(doc.id_documento_proveedor);
                              setSuccessMessage(`Documento "${doc.nombre_original}" eliminado.`);
                              cargarDocumentos();
                              setTimeout(() => setSuccessMessage(null), 3000);
                            } catch (err) { setErrorGlobal(`Error al eliminar: ${(err as Error).message}`); }
                          }
                        }}
