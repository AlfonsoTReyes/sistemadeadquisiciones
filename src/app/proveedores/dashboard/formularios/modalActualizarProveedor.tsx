// src/app/proveedores/dashboard/formularios/ModalActualizarProveedor.tsx
'use client';
...
// RepresentanteEditable interface removed, RepresentanteLegalOutput will be used directly.

// Datos del proveedor recibidos como props
...
// Estado interno del formulario principal
...
const ModalActualizarProveedor: React.FC<ModalProps> = ({
    ...
    proveedorData,
    onSubmit,
    isLoading, // This prop indicates if the parent is submitting
    error: apiError,
}) => {
    // --- Estados ---
    ...
    const [representantes, setRepresentantes] = useState<RepresentanteLegalOutput[]>([]);
    ...
    // Local isSubmitting state removed, using isLoading prop from parent

    // --- Efecto para inicializar/resetear estados ---
    useEffect(() => {
        // This effect runs when proveedorData changes, which is fine.
    ...
    // Early return if proveedorData is somehow null (though props type it as non-null)
    // This guard is mostly for type safety if the parent component doesn't adhere to types.
    if (!proveedorData) {
        console.error("ModalActualizarProveedor: proveedorData es null, no se puede renderizar.");
        return null;
    }

    // Tipo original (no editable), declared after the null check for proveedorData
    const tipoProveedorOriginal = proveedorData.tipo_proveedor;
    if (tipoProveedorOriginal !== 'moral' && tipoProveedorOriginal !== 'fisica') {
        // Handle 'desconocido' or invalid type if necessary, or default to one
        // For now, this will prevent rendering the specific form parts if type is unknown
        console.warn("Tipo de proveedor original es 'desconocido' o inválido:", tipoProveedorOriginal);
    }
    ...
    const handleAddRepresentante = () => {
        ...
        const nuevoRepresentante: RepresentanteLegalOutput = { // Using RepresentanteLegalOutput
    ...
    return (
        <div className="fixed inset-0 bg-gray-800 bg-opacity-80 flex justify-center items-center z-50 p-4 transition-opacity duration-300 ease-in-out">
            <div className="bg-white rounded-lg shadow-2xl p-6 md:p-8 w-full max-w-4xl max-h-[95vh] overflow-y-auto transform transition-all duration-300 ease-in-out scale-100">
                ...
                <form onSubmit={handleSubmit} noValidate>
                    <div className="mb-4">
                        ...
                        />
                        {/* Hidden input might not be necessary if tipoProveedorOriginal is used directly in payload */}
                    </div>
                            ...
                            <div className="mt-4 space-y-4">
                                ...
                                {representantes.length > 0 && (
                                    <div className="space-y-2 border-t pt-3 mt-3">
                                        {representantes.map((rep) => ( // index removed
                                            <div key={rep.id_morales} className="flex justify-between items-center bg-gray-100 px-3 py-2 rounded text-sm">
                                                ...
                                                <button type="button" onClick={() => handleRemoveRepresentante(rep.id_morales)} className="text-red-600 hover:text-red-800 text-xs disabled:opacity-50" title="Eliminar" disabled={isLoading}>Eliminar</button>
                                            </div>
                                        ))}
                                    </div>
                                )}
                    ...
                    <div className="mt-8 pt-5 border-t border-gray-200 flex justify-end space-x-4">
                        ...
                        <button
                            type="submit"
                            disabled={isLoading}
                            className={`inline-flex justify-center py-2 px-6 border border-transparent shadow-sm text-base font-medium rounded-md text-white ${isLoading
            ...
            <style jsx global>{`
                /* Asegúrate que estos estilos no causen problemas o sean necesarios */
