...
} from './formularios/fetchPartidas'; // Ajusta ruta

// Importa las interfaces (SelectOption ya no es necesaria)
import { CatalogoPartida, ProveedorPartidaSeleccionada } from './interface'; // Ajusta ruta
...
export default function GestionPartidasProveedorPage() {
    ...
    const [mostrarSoloSeleccionadas, setMostrarSoloSeleccionadas] = useState<boolean>(false); // <-- NUEVO ESTADO
    ...
    // 2. Efecto para cargar datos (Catálogo y Seleccionadas) (sin cambios)
    useEffect(() => {
        ...
        const cargarDatosIniciales = async () => {
            ...
            try {
                ...
                setErrorData(err.message || 'Error al cargar datos.');
    ...
    // 3. Filtrar catálogo por texto (sin cambios)
    ...
    // 4. Determinar qué partidas mostrar en la lista (filtrada por texto O solo seleccionadas)
    const partidasParaMostrar = useMemo(() => {
        if (mostrarSoloSeleccionadas) {
            // Filtrar el catálogo completo para mostrar solo las que están en partidasSeleccionadasCodigos
            return catalogoCompleto.filter(partida =>
                partidasSeleccionadasCodigos.includes(partida.codigo)
            );
        } else {
            // Si no, mostrar el catálogo filtrado por texto
    ...
    // 5. Handler para cambios en los checkboxes (sin cambios)
    ...
    // 6. Handler para guardar los cambios (sin cambios)
    const handleGuardarCambios = async () => {
        ...
        try {
            ...
            setErrorSync(err.message || "Error al guardar.");
        } finally {
            setLoadingSync(false);
        }
    };

    // 7. Handler para el nuevo checkbox "Mostrar solo seleccionadas"
    const handleMostrarSoloSeleccionadasChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        // Opcional: Limpiar el filtro de texto cuando se activa mostrar solo seleccionadas
