// src/app/menu.tsx (CORREGIDO Y CON NOTIFICACIONES INTEGRADAS)
...
import {
  ...
  faFileSignature, faFileContract, faGavel, faWarehouse, faBell // Añadido faBell
...
// Hook y Toaster
import { Toaster, toast } from 'react-hot-toast'; // Importar toast también si se usa directamente
...
export default function Menu() {
  ...
  const [idUsuario, setIdUsuario] = useState<string | null>(null); // Corregido: nombre consistente y tipo null
  ...
  useEffect(() => {
    ...
    const storedIdRol = sessionStorage.getItem("userRole") || null; // Cargar idRol
    ...
    // Validar sesión: necesita ID y permisos
    if (storedId && parsedPermissions.length > 0) {
      setEmail(storedEmail);
      setNombre(storedNombre);
      setIdUsuario(storedId); // Usar setIdUsuario consistentemente
      setIdRol(storedIdRol); // Guardar idRol
    ...
    <> {/* Fragmento para Toaster y Nav */}
      ...
      <nav className="bg-custom-color text-white w-full p-4 fixed top-0 z-50 shadow-md">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3"> {/* Reducido gap a 3 */}
            ...
            {/* Renderizar Notificaciones solo si idRol tiene valor */}
            {idRol && <Notificaciones idrol={idRol} />}
          </div>

          {/* Título Centrado */}
          {/* flex-1 asegura que tome el espacio restante, text-center lo centra */}
          ...
          <div className="w-16 md:w-20"></div> {/* Ajusta el ancho según necesites */}
        </div>
        ...
        >
          ...
          {/* (Secciones como USUARIO PROVEEDORES, ADMIN PROVEEDORES, ADQUISICIONES, etc.) */}
          {/* Usar la estructura de botón + ul condicional para cada sección */}

          {/* Ejemplo: Sección ADQUISICIONES */}
            ...
            <li className="mb-1">
              ...
              {isAdquisicionesOpen && (
                <ul className="ml-4 mt-1 space-y-1 bg-gray-700 rounded-md p-2">
                  ...
                  {/* Corregido: Permiso para tablas comparativas */}
                  {permissions.includes('menu_ver_tablas_comparativas') && ( <li className="mb-1"><Link href="/tablas_comparativas" onClick={() => setIsOpen(false)} className="flex items-center text-white hover:bg-[#faa21b] px-4 py-2 rounded-md"><FontAwesomeIcon icon={faClipboardCheck} className="mr-2 w-5" /> Tablas Comparativas</Link></li>)}
                  {/* Corregido: Permiso para admin proveedores */}
                  {permissions.includes('menu_ver_administrador_proveedores') && ( <li className="mb-1"><Link href="/adminProveedores/altaProveedor" onClick={() => setIsOpen(false)} className="flex items-center text-white hover:bg-[#faa21b] px-4 py-2 rounded-md"><FontAwesomeIcon icon={faUsers} className="mr-2 w-5" /> Admin Proveedores</Link></li>)}
                   {/* Corregido: Permiso para padrón */}
                  {permissions.includes('menu_ver_padron_proveedores') && ( <li className="mb-1"><Link href="/catalogoProveedores" onClick={() => setIsOpen(false)} className="flex items-center text-white hover:bg-[#faa21b] px-4 py-2 rounded-md"><FontAwesomeIcon icon={faClipboardList} className="mr-2 w-5" /> Padrón Proveedores</Link></li>)}
                   {/* Corregido: Permiso para admin pagos */}
          ...
          {/* Repetir estructura similar para COMITE, CONCURSOS, SECRETARIAS, FINANZAS */}
          {/* ... */}
          ...
          {email && idUsuario && ( // Usar idUsuario (el estado corregido)
            <li className="mb-1">
